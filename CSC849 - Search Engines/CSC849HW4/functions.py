...
def homework4_query(ii, fileArray):
    # battery screen speed
    added = []
    term_scores1 = []
    term_scores2 = []
    print("Query: battery")
    query1_relevantDocID = free_text_parsing("battery", ii)
    for file in fileArray:
        if file.docID == query1_relevantDocID:
            for term in file.normalized:
                if term not in added:
                    added.append(term)
                    thingToAdd = []
                    thingToAdd.append(score(term, file, ii))
                    thingToAdd.append(term)
                    term_scores1.append(thingToAdd)
    term_scores1 = list(reversed(sorted(term_scores1)))

    addedWords = select_top_x_terms(3, term_scores1)
    newQuery = "battery "
    for word in addedWords:
        newQuery += word + " "
    print("")
    print("New Query: ", newQuery)
    free_text_parsing(newQuery, ii)
    print("")
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    added = []
    print("Query: screen")
    query2_relevantDocID = free_text_parsing("screen", ii)
    for file in fileArray:
        if file.docID == query2_relevantDocID:
            for term in file.normalized:
                if term not in added:
                    added.append(term)
                    thingToAdd = []
                    thingToAdd.append(score(term, file, ii))
                    thingToAdd.append(term)
                    term_scores2.append(thingToAdd)
    term_scores2 = list(reversed(sorted(term_scores2)))

    addWords = select_top_x_terms(3, term_scores2)
    newQuery = "screen "
    ...
    # Worry about this Query later.
    print("Query: speed")
    newQuery = "speed "
    term_scores3 = []
    query3_relevantDocID = free_text_parsing("speed", ii)
    if invalidQuery == "777":
        print("\nQuery", newQuery, "not found during search.")
        print("Creating query of synonyms . . . \n")
        addWords = get_synonyms()
        for word in addWords:
            newQuery += word + " "
        print("New Query: ", newQuery)
        free_text_parsing(newQuery, ii)
        ...
        for file in fileArray:
            if file.docID == query3_relevantDocID:
                for term in file.normalized:
                    if term not in added:
                        added.append(term)
                        thingToAdd = []
                        thingToAdd.append(score(term, file, ii))
                        thingToAdd.append(term)
                        term_scores3.append(thingToAdd)
        term_scores3 = list(reversed(sorted(term_scores3)))

        addWords = select_top_x_terms(3, term_scores3)
        newQuery = "speed "
        for word in addWords:
            newQuery += word + " "
    print("")
...
def select_top_x_terms(number, lst):
    returnLst = []
    while number > 0:
        entry = lst[number - 1]
        returnLst.append(entry[1])
        number -= 1
...
def get_synonyms():
    return ["fast", "quick", "acceleration"]
